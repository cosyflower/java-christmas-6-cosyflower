...
- 크리스마스 디데이 할인 
  - **12.01 - 12.25일까지 진행한다**
  - 할인 금액의 시작은 100원이다
  - 하루를 지날수록 100원씩 할인 금액이 증가한다 ex) 100원 - 200원 ....
  - 25일 기준 해당 이벤트의 최대 할인 금액은 3400원이다 
  - 총 주문 금액에서 해당 할인 금액을 제외한다

- 평일 할인 (일-목요일)
  - **12.01 - 12.31**일까지 진행한다 
  - **일요일에서 목요일**까지 적용되는 할인이다
  - **디저트 메뉴**를 한 개당 2,023원 할인한다 
  
- 주말 할인 (금-토요일)
  - **12.01 - 12.31**일까지 진행한다
  - **금요일에서 토요일**까지 적용되는 할인이다
  - **메인 메뉴**를 한 개당 2,023원 할인한다

- 특별 할인 
  - **12.03, 10, 17, 24 ,25, 31** 총 6일만 진행한다 
  - **총 주문 금액에서 100원을 할인한다**

- 증정 이벤트 
  - **12.01 - 12.31**일까지 진행한다
  - **할인 전 총 주문 금액** 12만원 이상일 때 적용된다 
  - 샴페인 1개 (25,000원)을 증정한다 
...
모든 이벤트는 적용 기간, 할인 금액, 증정 여부를 포함한다 
- 적용 기간을 관리하는 Enum
- 할인 금액을 Wrapping 하는 클래스 by IntSupplier (할인 금액을 반환한다)
